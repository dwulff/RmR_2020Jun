...
In diesem Practical wirst du das Dashboard der letzten Aufgabe weiter ausbauen und interaktiver und dynamischer gestalten. Am Ende wirst du [dieses](https://therbootcamp.shinyapps.io/berlindashboard2/) Dashboard erstellen.
...
1. Öffne dein `TheRBootcamp` R project. Es sollte die Ordner `1_Data` und `3_Markdown` enthalten. Stelle sicher, dass du alle Datensätze, welche im `Datensätze` Tab aufgelisted sind, in deinem `1_Data` Ordner hast. 

2. Lade [dieses](https://therbootcamp.github.io/RmR_2020Jun/_sessions/Dashboards/app/BerlinDashboard.Rmd) Markdown Skript herunter und speichere es in deinen `3_Markdown` Ordner.
...
5. Nein, wir müssen zuerst Inputs definieren. Dazu verwenden wir eine Sidebar, indem alle User Inputs gemacht werden. Füge dazu noch vor dem Titel der ersten Seite (also vor `Übersicht der Airbnbs in Berlin {data-orientation=rows}`) eine neue Seite ein und nenne diese `Inputs` (erinnere, dass man Seiten mit einem Titel und einer Zeile Gleichheitszeichen definiert).

6. Hinter das `Inputs` (aber auf derselben Zeile), schreibe `{.sidebar}`. Damit wird eine Sidebar definiert, die über alle drei Seiten der App fix auf der linken Seite bestehen bleibt. 

7. Zeit, die neu erstellte Sidebar zu füllen. Erstelle dazu unterhalb des `Inputs` Titels (also auch unterhalb der Gleichheitszeichen) einen neuen R chunk.
...
16. Wiederhole die Schritte B14 und B15 auch noch für die letzten chunks (zweimal die Plots und zweimal die Gauges, in abwechselnder Reihenfolge).
...
21. Da wir nun das `farben_dynamic()` Objekt oben definiert haben, wird das `farben` Objekt nicht mehr benötigt. Lösche dessen Definition oberhalb des Codes für die Häufigkeitsverteilung:
...
22. Ersetze ausserdem im Code der Häufigkeitsverteilung (dem `plotly` Plot) das Objekt `farben` durch `farben_dynamic()`. **Tipp:** Du findest das Objekt in der Zeile `scale_color_manual(values = farben) +`.
...
Untenstehend findest du ein paar Vorschläge, wie du dein Dashboard weiter aufpeppen und erweitern könntest. Versuche eigenständig weiter Features einzubauen. Dazu kannst du auf die Vorschläge unten zurückgreifen, kannst aber auch gerne eigene Ideen umsetzen; je nach dem was dich am meisten reizt.
...
4. Keine der obigen Optionen spricht dich an (oder vielleicht hast du ja bereits alle gemacht)? Überlege dir, wie du das Dashboard weiter aufpeppen kannst. Das kann das Styling betreffen, die Inputs, die angezeigte Information etc. etc... Wir sind gespannt auf deine Kreation!
