...
1. Erstelle ein neues Unterkapitel mit der Überschrift: `Einfluss Ausstattung` (als Level 4 Header).
...
1. Erstelle einen neuen Code Chunk mit einer Tabelle basierend auf `ausstattung`. Verwende dafür die `kable()` Funktion.
...
2. Beschrännke die Anzahl Nachkommastellen mit `digits = 2` auf zwei.

3. Füge eine Caption mit `caption = 'Tabelle 1. Austattung und Einfluss auf den Preis'` ein.
...
5. `Knitte` das Dokument. Die Spaltenbeschriftung wird nicht korrekt übersetzt. 
Füge `escape = FALSE` um dieses Problem zu korrigieren. 

6. Lege die Ausrichtung der Zellen mit `align="lcccc"` fest.

7. Nun fixiere die Breite der Spalten mit `column_spec(c(2,3,4,5), width = "1.2in")`.

8. Die Schrifgrösse der Zellen soll in Abhängigkeit der Grösse der abgebildeten Zahl unterschiedlich sein. 
Füge dafür vor dem `kable` Abschnitt folgenden Code ein und verbinde die beiden Teile mit einer Pipe `%>%`.
...
1. Erstelle ein neues Unterkapitel mit der Überschrift: `Regressionsmodell` als Level 4 Header

2. Berechne eine Regression in der `Preis` mit verschiednen Ausstattungsmerkmalen vorhergesagt wird sowie den  durchschnittlichen Anteil der Hotels. Diesen Chunk nicht ausgeben!
...
3. Füge folgenden Textblock ein.
...
4. Berechne den Koeffizienten aus dem Regressionsmodell im 4. Absatz (Hotels haben einen um ... höheren Preis) mit folgendem inline code und zeige das Ergebnis im Satz an.

```{r echo = TRUE}
coefficients(ausstattung_mod)['HotelTRUE'] %>% round(1)
```

5. Hebe den 2. ("Überraschenderweise ...") und 4. Absatz ("Der Anteil ...") fett hervor.

6. `Knitte` das Dokument. Sieht alles in Ordnung aus?  

### H - Preise: Regressionstabelle

1. Zeige die Regressionsergebnisse in einer Tablle mit `sjPlot::tab_model` an.
...
2. Ersetze `Intercept` mit `Achsenabschnitt` in der Tabelle: `pred.labels = c('Achsenabschnitt', names(ausstattung)[-1])`

3. Füge einen neuen Tabellentitel ein: `title = 'Table 2. Regression des Preises auf die Ausstattung.'`
...
|:---|:------|:---------------------------------------------|
| `pull`|`dplyr`|Extrahieren eines Vektors| 
...
- [**CSS**](https://www.w3schools.com/css/) Dokumentation.
