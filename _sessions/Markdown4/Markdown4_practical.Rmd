...
3.  Aus dem Abschnitt "Entwicklung" in `3_Markdown/airbnb_final.Rmd` übertrage die Anzahl der eingestellten Wohnungen in den ersten Bullet Point. 
...
2. Mit Hilfe einer HTML Tabelle erstelle ein Layout mit 2 Spalten. Liste in der linken Spalte die Stadtteile auf (mit `inline` Code) und in der rechten Spalten die prozentuellen Veränderungen. 

3. Stelle auf dem nächste Slide (gleicher Titel wie zuvor) die Abbildung 2 (Neueinstellungen pro Bezirk) als Grafik dar. Achte darauf, dass alle R Chunks (auch jene mit Berechnungen) mitkopiert werden.
...
5. Optional: Füge nun Sterne (anstatt der p-Werte) für signifikante Estimates ein (Hint: `p.style`)
