...
2. Erstelle eine Bullet Point Liste. In Markdown wird ein `-` in einen Bullet Point übersetzt.  

3.  Aus dem Abschnitt "Entwicklung" in `3_Markdown/airbnb_final.Rmd` übertrage die Anzahl der eingesetellten Wohnungen in den ersten Bullet Point. 

4. Erstelle einen weiteren Bullet Point mit den Wohnungseinstellungen für das Jahr 2018 sowie das Jahr 2019.

5. Dokumentiere im vierten Bullet Point die durchschnittliche Verfügbarkeit in den nächsten 3 Monaten. 
...
3. Vergiss nicht in den Chunk Optionen `fig.cap = cap` einzufügen, damit die Bildunterschrift angezeigt wird.
...
2. Mit Hilfe einer HTML Tabelle erstelle ein Layout mit 2 Spalten. Liste in der linken Spalte die Stadtteile auf (mit `inline` Code) und in der rechten Spalten die prozentuellen Veränderunge. 

3. Stelle auf dem nächste Slide (gleicher Titel wie zuvor) die Abbildung 2 (Neueinstellungen pro Bezirk) als Grafik dar. Achte darauf, dass alle R Chunkgs (auch jene mit Berechnungen) mitkopiert werden.
...
4. Auf dem gleichen Slide, unter der Tabelle, füge den letzten Satz des Abschnittes "Der Einfluss der Ausstattung" zum stark negativen Effekt der Küche ein.

5. Knitte das Dokument. Wird die Tabelle korrekt angezeigt?

6. Optional: Veränder die Tabelle so, dass die Schriftgrösse in allen Zellen gleich gross ist und dass Zellen mit einer Zahl > 100 rot angezeigt werden (Hinweis: `color = ifelse(x > 100, "red", "black")`)
...
4. Optional: Mit dem Attribut `col.order` kannst du die Reihenfolge und Art der Ergebnisse in der Regressionstabelle festlegen. Verändere die Reihenfolge der Spalten, sodass zuerst "Konfidenzintervalle" (ci) und dann die "Estimates" angezeigt werden.

5. Optional: Füge nun Sterne für signifikante Estimates ein (Hint: `p.style`)
