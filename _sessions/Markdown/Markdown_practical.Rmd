...
3. Tausche zunächst das Datum an zweiter Stelle im Text durch den folgenen inline Code aus. Nicht vergessen, der Code muss innerhalb zweier Ticks gesetzt werden. Knitte das Dokument.  

````text
r min(airbnb_1819$Erstellungsdatum)
````

4. Jetzt sollte der Text ein Datum enthalten, welches aber noch nicht ganz richtig formatiert ist. Hier hilft die `strftime()` Funktion. Versuche den folgenden erwarteten inline Code um das Datum in das richtige Format zu bringen.   
...
7. Im nächsten Paragraphen ersetze die ersten beiden Zahlen durch `n_2019 <- sum(year(airbnb_1819$Erstellungsdatum) == 2019); n_2019` und `n_2018 <- sum(year(airbnb_1819$Erstellungsdatum) == 2018); n_2018`. In diesen beiden Fällen siehst du, dass auch Zuweisungen in inline Code möglich sind. 

8. Ersetze nun die Zuwachsangabe am Ende des Paragraphen durch `(round(n_2019/n_2018, 2) - 1)*100`, was , wie du siehst, auf die beiden zuvor im inline Code definierten Objekte zurückgreift. Knitte das Dokument und überprüfe, ob nun alle Zahlen einwandfrei wiedergegeben werden.

9. Zuletzt verwende `**` um den gesamten zweiten Paragraphen Fett zu setzen. Setze hierfür einmal die zwei Sterne vor das erste und nach dem letzten Wort des Paragraphen. Nun alles wie im Zieldokument?

### D - Neueinstellungen: Grafikchunk pt. 1

1. Erstelle einen neuen R `chunk`. Einen `chunk` erstellst du mit zunächst drei Ticks, dann direkt danach in der selben Zeile `{r}` und anschliessend nocheinmal drei Ticks in einer Zeile darunter. Wie in der Abbildung unten sollte sich der Hintergrund automatisch grau einfärben. 

<p align="center"><img src="image/chunk.png" height=60px></img></p>

2. Kopiere in den `chunk`, d.h., zwischen die zwei Zeilen mit den drei Ticks, den Code unten, welcher eine erste Version der ersten Abbildung im Bericht generiert. 
...
1. Aktuell zeigt dein "geknittetes" Dokument sowohl den Code für den Plot als auch den Plot selbst. Das Dokument soll aber nur den Plot zeigen. Ergänze in den `chunk` Optionen, d.h., innerhalb der geschwungenen Klammern nach dem `r`, `echo = FALSE`. Danach knitte das Dokument nocheinmal.  

<p align="center"><img src="image/chunk2.png" height=60px></img></p>

2. Das Dokument sollte nun den Code in dem `chunk` nicht mehr zeigen. Mache nun dieses Setting rückgängig, d.h., lösche `echo = FALSE`, es gibt nämlich einen zweiten Weg diesen und alle zukünftigen Code `chunk`s zu verstecken. Knitte das Dokument nachdem du das `echo = FALSE` entfernt hast. 

3. Gehe nun zum Setup `chunk` ganz am Anfang es Dokuments (unter dem YAML head). Dort siehst du folgende Code Zeile `knitr::opts_chunk$set(echo = TRUE)`, die aktuell impliziert, dass alle Code `chunk`s per default angezeigt werden. Setze nun hier `echo = FALSE` und knitte das Dokument. Der Code `chunk` sollte nun wieder versteckt sein.    
...
5. Kreiere einen neuen Chunk zwischen den beiden Paragraphen mit dem folgenden Code, welcher einige Objekte erstellt, die die inline Ergänzungen im darauf folgenden Paragraphen erleichtern werden.  
...
6. Ersetze die ersten beiden Stadtteil-Nennungen in beiden Sätzen durch `min_stadtteil` und `max_stadtteil` und knitte das Dokument.
...
### G - Neueinstellungen: Grafikchunk pt. 2

1. Erstelle einen neuen `chunk` mit dem folgenden Code für die zweite Abbildung.
