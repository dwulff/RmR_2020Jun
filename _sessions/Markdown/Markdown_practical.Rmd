...
2. In-line Code im Text verwendet. 
...
2. Füge nun diesen Text ein. Er enthält einige Werte, die du gleich durch in-line Code ersetzen wirst.
...
7. Im nächsten Paragraphen ersetze die ersten beiden Zahlen durch `n_2019 <- sum(year(airbnb_1819$Erstellungsdatum) == 2019); n_2019` und `n_2018 <- sum(year(airbnb_1819$Erstellungsdatum) == 2018); n_2018`. In diesen beiden Fällen siehst du, dass auch Zuweisungen in in-line Code möglich sind. 

8. Ersetze nun die Zuwachsangabe am Ende des Paragraphen durch `(round(n_2019/n_2018, 2) - 1)*100`, was , wie du siehst, auf die beiden zuvor im in-line Code definierten Objekte zurückgreift. Knitte das Dokument und überprüfe, ob nun alle Zahlen einwandfrei wiedergegeben werden.
...
5. Kreiere einen neuen Chunk zwischen den beiden Paragraphen mit dem folgenden Code, welcher einige Objekte erstellt, die die in-line Ergänzungen im darauf folgenden Paragraphen erleichtern werden.  
