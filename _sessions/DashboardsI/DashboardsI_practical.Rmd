...
In diesem Practical übst du, (dynamische) Dashboards in Markdown zu erstellen. Genauer wirst du [dieses](https://therbootcamp.github.io/RmR_2020Jun/_sessions/DashboardsI/app/BerlinDashboard.html) Dashboard erstellen.
...
1. Öffne dein `TheRBootcamp` R Projekt. Es sollte die Ordner `1_Data` und `3_Markdown` enthalten. Stelle sicher, dass du alle Datensätze, welche im `Datensätze` Tab aufgelistet sind, in deinem `1_Data` Ordner hast. 
...
2. Lade den `airbnb` Datensatz mit `read_csv()` ebenfalls im `setup` chunk, so wie du das in den bisherigen Practicals auch gemacht hast (die beiden Punkte am Anfang verlegen den Pfad um einen Ordner zurück).
...
7. Wenn du das Dokument so knitten würdest, erhielten die beiden Zeilen gleich viel Platz. Die Karte soll jedoch mehr Platz erhalten. Füge dazu nach dem ersten Titel (`Zeile 1`, aber auf derselben Zeile im Code) folgenden Code ein: `{data-height = 400}`. Damit wird die Zeilenhöhe in Pixeln angegeben; Zeile 2 füllt dann den restlichen Platz aus (**Hinweis:** Die Platzverteilung erfolgt erst, wenn die Komponenten gefüllt werden; wenn du das Dokument jetzt renderst, wird also Zeile 1 im Moment noch gleich hoch sein, wie Zeile 2).

8. Zeile 1 mit den value Boxes, ist wiederum in drei Spalten unterteilt, eine für jede value Box. Verwende Titel dritter Ordnung um die Spalten mit den Namen "Superhosts", "Allstar", und "Preiswert" zu erstellen. Für diese Titel werden keine besonderen Zeichen verwendet, sondern wie gewohnt `###`.

9. In Zeile 2 (also unterhalb des nächsten Titels zweiter Ordnung) musst du nur einen Titel einfügen, ohne Titel dritter Ordnung klappt nähmlich die automatische Grössenanpassung nicht. Gebe "Karte" als Titel an, und füge direkt dahinter `{.no-title}` ein; damit wird die Präsentation des Titels auf dem Dashboard unterdrückt.
...
12. Genau, du kannst wie bei Aufgabe C6 Titel zweiter Ordnung hinzfügen. Füge die zwei Spalten, diesmal mit den Titeln "Spalte 1" und "Spalte 2", unter dem Titel der zweiten Seite hinzu (also nach der Zeile mit den Gleichheitszeichen).
...
18. Fast geschafft! Nur die Titel der Storyboardabschnitte sind noch etwas klein. Um sie zu vergrössern, musst du etwas HTML Syntax verwenden. Kopiere den untenstehenden Code und ersetze die Titel damit, und füge die Titelnamen von Aufgabe C16 in die `XXX` des Codes ein.
...
5. Funktioniert, aber ein paar Dinge fehlen noch. Zunächst stimmt die Farbe nicht. Füge in die `valueBox()` Funktion ein Argument `color` ein und setze dieses auf `"#00A699"`.
...
10. Schaue dir den Text in der rechten Spalte des [Dashboards](https://therbootcamp.github.io/RmR_2020Jun/_sessions/DashboardsI/app/BerlinDashboard.html) an. Versuche ihn mit deinen bisher erworbenen RMarkdown Kenntnissen selbst nachzubauen (du darfst copy-pasten um nicht alles abschreiben zu müssen).

11. Du kannst deinen Text mit dem untenstehenden Text, mit dem diese Spalte tatsächlich erstellt wurde, vergleichen (**Hinweis:** Die einfachen Zeilenumbrüche sind nur eingefügt, um alles ohne seitwärts scrollen zu müssen anzeigen zu können). Hast du alles gleich gemacht? Wo hast du andere Lösungen gefunden?
...
es in Berlin-Pankow alleine `r knitr::inline_expr("round(10**2.3)")` Airbnbs. 
Mittlerweile ist diese Zahl auf rund `r knitr::inline_expr("round(10**3.2 / 100) * 100")` angestiegen.

Das teuerste heute verfügbare Airbnb kostet dabei `r knitr::inline_expr("max(airbnb$Preis)")` 
pro Nacht; für die meisten Leute nicht gerade ein Schnäppchen. Dabei
handelt es sich um ein `r knitr::inline_expr("airbnb$Unterkunftsart[max(airbnb$Preis)]")` 
im Stadtteil `r knitr::inline_expr("airbnb$Stadtteil[max(airbnb$Preis)]")`.

Insgesamt wurden über die Jahre in Berlin `r knitr::inline_expr("as.integer(sum(airbnb$Host_anzahl))")`
...
16. Um den `gauge` zu erstellen, berechne zuerst das mittlere Gesamtrating. Erstelle dazu einen neuen R chunk und füge den untenstehenden Code ein. 
...
17. Mit diesem Objekt kannst du jetzt den `gauge` zu erstellen. Verwende das untenstehende Template und setze `gesamtrating` als erstes Argument ein.
...
18. Wenn du das Dokument jetzt knittest, siehst du, dass die Dimensionen in der Kommentarspalte noch falsch sind. Das ist, weil der `gauge` noch zu gross ist. Um das zu beheben, kannst du `fig.width=3,fig.height=1.5, fig.align="center"` in die R chunk Optionen des chunks einfügen, welcher den `gauge` erstellt.

19. Füge nun unterhalb des R chunks, der den `gauge` definiert, den folgenden Text ein (der Teil `<p style="font-size: 20px;">` ist HTML Syntax, mit der die Textgrösse etwas vergrössert wird; mit `</p>` wird die Syntax abgeschlossen):
(**Hinweis:** Die Zeilenumbrüche sind nur eingefügt, um alles ohne seitwärts scrollen zu müssen anzeigen zu können)
...
1. Wir starten mit der Karte auf Seite 1. Gehe in deinem Skript zur Syntax, welche die Zeile 2 auf der ersten Seite erstellt (die Zeile unterhalb der value Boxen).

2. [leaflet](https://rstudio.github.io/leaflet/) bietet die Möglichkeit, sehr einfach interaktive Karten zu erstellen. Erstelle einen neuen R chunk mit dem folgenden Code unterhalb des "Karte" Titel:
...
4. Bzw. fast fertig. Passe die Dimensionen des Plots (`fig.width` und `fig.height` R chunk Optionen) so an, dass die Spalte möglichst ausgefüllt wird. Dazu musst du evtl. ein paar Werte ausprobieren...
