...
farben <- switch(input$Farbpalette,
...
renderPlot({
  ...
  pg2 <- airbnb %>% 
    filter(Stadtteil %in% c(input$Stadtteile) &
           Schlafzimmer >= as.numeric(input$Schlafzimmer)) %>%   
    ggplot(aes(x = log10(Preis), y = Rating_gesamt)) +
...
renderPlot({
  ...
  ps2 <- airbnb %>% 
      filter(Stadtteil %in% c(input$Stadtteile) &
             Schlafzimmer >= as.numeric(input$Schlafzimmer)) %>% 
    ggplot(aes(x = log10(Preis), y = Rating_sauberkeit)) +
    geom_point(alpha = .25) +
    labs(x = bquote("log"[10]*"(Preis)"),
         y = "Sauberkeitsrating") +
...
renderPlot({
  ...
  pl2 <- airbnb %>% 
        filter(Stadtteil %in% c(input$Stadtteile) &
               Schlafzimmer >= as.numeric(input$Schlafzimmer)) %>%
    ggplot(aes(x = log10(Preis), y = Rating_lage)) +
    geom_point(alpha = .25) +
    labs(x = bquote("log"[10]*"(Preis)"),
         y = "Lagerating") +
    coord_cartesian(ylim = c(0, 10)) +
